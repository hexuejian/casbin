...
func TestLoadFilteredPolicy(t *testing.T) {
	...
	if !e.IsFiltered() {
		t.Errorf("beegoormadapter did not set the filtered flag correctly")
	}
	...
	if err := e.GetAdapter().SavePolicy(e.GetModel()); err == nil {
		t.Errorf("beegoormadapter did not prevent saving filtered policy")
	}
}
...
func TestFilteredPolicyEmptyFilter(t *testing.T) {
	...
	if e.IsFiltered() {
		t.Errorf("beegoormadapter did not reset the filtered flag correctly")
	}
	if err := e.SavePolicy(); err != nil {
